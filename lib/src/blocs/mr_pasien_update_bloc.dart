...
class MrPasienUpdateBloc {
  final _repo = MrPasienUpdateRepo();
  StreamController<ApiResponse<PasienShowModel>>? _streamPasienSave;
  ...
  StreamSink<ApiResponse<PasienShowModel>> get pasienSaveSink =>
      _streamPasienSave!.sink;
  Stream<ApiResponse<PasienShowModel>> get pasienSaveStream =>
      _streamPasienSave!.stream;

  Future<void> updatePasien() async {
    _streamPasienSave = StreamController();
    final idPasien = _idPasien.value;
    ...
    try {
      final res =
          await _repo.updatePasienBaru(mrPasienSaveRequestModel, idPasien);
